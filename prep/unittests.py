...
class PatchExtractorTests(unittest.TestCase):
    ...
    def test_get_patches_from_image(self):
        ...
        self.assertTrue(
            np.all(extracted_patches[p1] == im[p1[0] - 63 : p1[0] + 64, p1[1] - 63 : p1[1] + 64, :]),
            "Incorrect patch values",
        )
        self.assertTrue(
            np.all(extracted_patches[p2] == im[p2[0] - 63 : p2[0] + 64, : p2[1] + 64, :]), "Incorrect patch values"
        )
        self.assertTrue(
            np.all(extracted_patches[p3] == im[p3[0] - 63 : p3[0] + 64, p3[1] - 63 : p3[1] + 64, :]),
            "Incorrect patch values",
        )
        self.assertTrue(
            np.all(extracted_patches[p4] == im[p4[0] - 63 :, p4[1] - 63 : p4[1] + 64, :]), "Incorrect patch values"
        )

        # Test that the means of the first channel (Blue, given opencv reads in BGR format) have not changed.
        self.assertAlmostEqual(
            extracted_patches[p1][:, :, 0].mean(), 49.5024490049, places=3, msg="Spot check on channel mean fail"
        )
        self.assertAlmostEqual(
            extracted_patches[p2][:, :, 0].mean(), 86.4618041166, places=3, msg="Spot check on channel mean fail"
        )
        self.assertAlmostEqual(
            extracted_patches[p3][:, :, 0].mean(), 45.5314030628, places=3, msg="Spot check on channel mean fail"
        )
        self.assertAlmostEqual(
            extracted_patches[p4][:, :, 0].mean(), 72.5908725695, places=3, msg="Spot check on channel mean fail"
        )

        # Save test patches with fake labels
        labels = pd.Series([2, 238, 2, 238], index=coords_index, name="label")
        prep.patches.write_patches_as_images(
            TEST_IMAGE_NAME, patches=extracted_patches, labels=labels, out_dir=TEST_OUTPUT_DIR
        )
    ...
    def test_dropping_cropped_patches(self):
        ...
        p4 = (989, 365)
        coords_index = pd.MultiIndex.from_tuples([p1, p2, p3, p4], names=["row", "col"])
        extracted_patches = prep.patches.get_patches_from_image(
            im=im, coords_index=coords_index, patch_size=PATCHSIZE, discard_cropped=True
        )
        # Correct number of patches
